...
class Handler {
    constructor(client) {
        ...
        this.baseRequest = axios.create({
            ...
            httpAgent: new http.Agent({ keepAlive: true, maxSockets: this.options.overrides.maxSockets || 2 }),
            httpsAgent: new https.Agent({ keepAlive: true, maxSockets: this.options.overrides.maxSockets || 2 }),
        });
    }
    ...
    async downloadAsync(url, directory, name, retry, type) {
        try {
            ...
            this.client.emit(
                'debug',
                `[MCLC]: Failed to download asset to ${path.join(directory, name)} due\n${error}. Retrying... ${retry}`,
            );
    ...
    async getVersion() {
        try {
            ...
            fs.writeFileSync(path.join(cache, 'version_manifest.json'), JSON.stringify(data));
            ...
            if (desiredVersion) {
                ...
                fs.writeFileSync(path.join(`${cache}/${this.options.version.number}.json`), JSON.stringify(data));
    ...
    async getAssets() {
        ...
        const index = JSON.parse(
            fs.readFileSync(path.join(assetDirectory, 'indexes', `${assetId}.json`), { encoding: 'utf8' }),
        );
    ...
    async getNatives() {
        const nativeDirectory = path.resolve(
            this.options.overrides.natives || path.join(this.options.root, 'natives', this.version.id),
        );
    ...
    async getClasses(classJson) {
        ...
        const libraryDirectory = path.resolve(
            this.options.overrides.libraryRoot || path.join(this.options.root, 'libraries'),
        );
    ...
    async getLaunchOptions(modification) {
        ...
        const assetPath = this.isLegacy() ? path.join(this.options.root, 'resources') : path.join(assetRoot);
